...
- **Auto-notifications** — fire-and-forget posts to the updates channel when key actions happen: event approved (@here ping), event cancelled, event edited (date/time/game changes), tournament created, poll created, new member joined. Each type individually toggleable. Rich embeds with color-coded borders (blue for events, green for approvals, red for cancellations, gold for tournaments, purple for polls, teal for new members), emoji icons, and CTA footers. Every notification includes a clickable link to `pvpers.us/schedule` (or `pvpers.us/polls` for poll notifications) so users can jump straight to the site. Event creation does not auto-post — the approval is the "it's official" moment.
...
- **5 announcement templates** — Game Night, Tournament, Big Event, Reminder, Hype Up, plus Custom. Templates interpolate event data into natural-sounding messages and include a `pvpers.us/schedule` link at the bottom. Quick Edit fields let admins tweak game/day/time/host without rewriting the message. Editing the title or body directly auto-switches to Custom. Announcement embeds link their title to the schedule page for one-click access.
