...
interface FeedbackProps {
  mySuggestions: { id: string; type: string; title: string; description: string | null; status: string; createdAt: string }[];
  isMuted: boolean;
  isLoggedIn: boolean;
}

type Tab = "About" | "Changelog" | "FAQ" | "Feedback";
const TABS: Tab[] = ["About", "Changelog", "FAQ", "Feedback"];

interface AboutPageClientProps extends FeedbackProps {
  initialTab?: Tab;
}

export default function AboutPageClient({ mySuggestions, isMuted, isLoggedIn, initialTab }: AboutPageClientProps) {
  const [activeTab, setActiveTab] = useState<Tab>(
    initialTab && TABS.includes(initialTab) ? initialTab : "About"
  );
  ...
  return (
    <div className="mx-auto max-w-3xl px-4 py-20">
      ...
      >
        ...
        {activeTab === "Feedback" && (
          <FeedbackContent mySuggestions={mySuggestions} isMuted={isMuted} isLoggedIn={isLoggedIn} />
        )}
      </motion.div>
    </div>
  );
...
const VERSION_HISTORY = [
  {
    ...
    changes: [
      ...
      "Feedback moved to About page — new Feedback tab with deep linking support, replaces profile section",
      "Profile feedback link — compact card on profile page directs to About > Feedback",
      "Admin type badges — suggestions tab shows type alongside status for each item",
    ],
  },
  {
    version: "v0.8.0",
    ...
    changes: [
      ...
      "Mute system — admins can mute users (permanent or timed), blocking suggestions, polls, and comments",
      "FAQ accordion — collapsible Q&A section on About page with smooth animations",
      "Comment improvements — enhanced poll comment system with better styling",
    ],
  },
  {
    version: "v0.7.1",
  ...
  },
  {
    ...
    changes: [
      ...
      "Accent color theming — 12 presets or custom hex, drives all neon highlights and glows",
      "Community branding — configurable name, tagline, logo, and favicon",
      "Join modes — open, invite-only (with code management), or approval (with admin queue)",
      "Feature toggles — master switches for tournaments, teams, polls, highlights, and stats",
    ],
  },
  {
    version: "v0.6.0",
...
const FAQ_ITEMS = [
  ...
  },
  {
    ...
    answer: "Yes! Head to the Feedback tab on this page. You can submit suggestions for new features or report bugs you've found, and admins will review them.",
  },
];
...
function FeedbackContent({ mySuggestions, isMuted, isLoggedIn }: FeedbackProps) {
  return (
    <motion.section variants={staggerItem}>
      <h2 className="mb-2 text-xl font-bold text-foreground">Suggestions & Bug Reports</h2>
      <p className="mb-6 text-sm text-foreground/50">
        Have an idea to improve the site or found something broken? Let us know below.
      </p>

      {isLoggedIn ? (
        <div className="rounded-xl border border-border bg-surface p-6">
          <FeedbackBox mySuggestions={mySuggestions} isMuted={isMuted} />
        </div>
      ) : (
        <div className="rounded-xl border border-border bg-surface p-6 text-center">
          <p className="text-foreground/50">
            Sign in to submit suggestions and bug reports.
          </p>
        </div>
      )}
    </motion.section>
