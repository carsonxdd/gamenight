...
import { SUGGESTION_STATUS_CONFIG, SUGGESTION_LIMITS } from "@/lib/suggestion-constants";
...
export default function ProfileSuggestionBox({ mySuggestions, isMuted }: Props) {
  ...
  const handleSubmit = async () => {
    ...
    const result = await createSuggestion(title.trim(), description.trim() || undefined);
    ...
    if (result.error) {
      ...
      setSuggestions((prev) => [
        {
          id: Date.now().toString(), // temp ID, will refresh on next load
          title: title.trim(),
    ...
    <div className="space-y-4">
      {isMuted ? (
        <div className="rounded-lg border border-danger/30 bg-danger/5 px-4 py-3 text-sm text-danger">
          You are currently muted and cannot submit suggestions.
        </div>
      ) : (
